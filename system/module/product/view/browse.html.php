  ...
  <div class='col-md-9'>
    <div class='box radius'>
      ...
      <li class='media f-left'>
        ...
        {
            echo html::a(inlink('view', "id=$product->id&currentCategory={$category->id}", "name=$product->alias"), html::image($themeRoot . 'default/images/main/noimage.gif', "title='{$title}' class='adaptive'"), '', "class='media-image'");
        }
        else
        {
            echo html::a(inlink('view', "id=$product->id&currentCategory={$category->id}", "name=$product->alias"), html::image($product->image->primary->smallURL, "title='{$title}' class='adaptive' alt='{$product->name}'"), '', "class='media-image'");
        }
        ?>
        <div class='media-body'>
          <h5 class='media-heading'><?php echo html::a(inlink('view', "id={$product->id}&currentCategory={$category->id}", "name=$product->alias"), $product->name);?></h5>
